...
if __name__ == "__main__":
    time_logger = init_log("global", logging.INFO)
    ...
    for step in range(1, config.num_iters + 1):
        if step % info_gap == 0:
            time_logger.info(f'running {step}/{config.num_iters}')
        # lr update
        ...
        # train a iteration
        train_all(net, config, loader_iter, optimizer, criterion, logger, time_logger, step)
        # test
        if step % 100 == 0:
            ...
            print(f'step={step}, mAP={test_info["average_mAP[0.1:0.3]"][-1]}, best mAP={best_mAP}')
    time_logger.info(f'best_step={best_step}')
    pprint(test_info)
